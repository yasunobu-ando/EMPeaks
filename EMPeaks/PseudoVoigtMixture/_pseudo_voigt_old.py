...
class PseudoVoigt:
    ...
    def _LogZ_m(self, m, g):
        """
        logarithmic derivative of Lorentian normalization factor by mean x0. 
        """
    ...
    def _LogZ_g(self, m, g):
        """
        logarithmic derivative of Lorentian normalization factor by FWHM gamma. 
        """
        _a = self.interval[0]
        _b = self.interval[-1]
    ...
    def full_optimization_old(self, x, intensity, eps, max_iter):
    ...
    def full_optimization(self, x, intensity):
            ...
            init = [self.x0, self.gamma, self.eta]
            ll = [self._LL(x, intensity)]
            info = optimize.minimize(func_ll, x0=init,
                                    bounds=[(self.x_min, self.x_max), (self.gamma_min, self.gamma_max), (0.5, 1.0)],
                                    method='L-BFGS-B')
            self.x0 = info['x'][0]
            self.gamma = info['x'][1]
            self.eta = info['x'][2]
    ...
    def full_optimization_fix_x0(self, x, intensity):
        ...
        ll = [self._LL(x, intensity)]
        info = optimize.minimize(func_ll, x0=init,
                                bounds=[(self.gamma_min, self.gamma_max), (0.5, 1.0)],
                                method='L-BFGS-B')
        self.gamma = info['x'][0]
        self.eta = info['x'][1]
    ...
    def full_optimization_fix_gamma(self, x, intensity):
        ...
        init = [self.x0, self.eta]
        ll = [self._LL(x, intensity)]
        info = optimize.minimize(func_ll, x0=init,
                                bounds=[(self.x_min, self.x_max), (0.5, 1.0)],
                                method='L-BFGS-B')
        self.x0 = info['x'][0]
        self.eta = info['x'][1]
    ...
    def _LL_m(self, x,intensity):
        ...
        _A = self.eta * self._N_m(x) + (1 - self.eta) * self._L_m(x)
        _B = intensity/self.predict_2(x)
        return (_B * _A).sum()

    def _LL_g(self, x,intensity):
        """ derivative of LL in respect to gamma """
        _A = self.eta * self._N_g(x) + (1 - self.eta) * self._L_g(x)
        _B = intensity/self.predict_2(x)
        return (_B * _A).sum()
    ...
    def _LL_eta(self, x,intensity):
        ...
        _sigma = self.gamma / (2.0 * np.sqrt(2.0 * np.log(2.0)))
        _A = norm.pdf(x, self.x0, _sigma) - cauchy.pdf(x, self.x0, self.gamma / 2.0)
        _B = intensity/self.predict_2(x)
        return (_B * _A).sum()
    ...
    def _LL_2(self, x, intensity):
        t = np.log(self.predict_2(x) + 1.0e-20)
        self.LL = (t * intensity).sum()
        return self.LL
    
    def _N_m(self,x):
        """ derivative of Gaussina in respect to x_0 """
        _sigma = self.gamma / (2.0 * np.sqrt(2.0 * np.log(2.0)))
        return norm.pdf(x, self.x0, _sigma) * 2.0 * np.ln(2.0)*(x - self.x0)/(self.gamma/2)**2
    
    def _N_g(self,x):
        """ derivative of Gaussina in respect to gamma """
        _sigma = self.gamma / (2.0 * np.sqrt(2.0 * np.log(2.0)))
        return 1.0/self.gamma^3 * (-self.gamma**2 + 8.0*np.ln(2.0)*(x-self.x_0)**2) \
