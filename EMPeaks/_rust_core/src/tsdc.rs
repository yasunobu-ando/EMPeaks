...
pub fn predict_tsdc(t: &[f64], ea: f64, tau0: f64, beta: f64) -> Vec<f64> {
    let unnorm: Vec<f64> = t.iter().map(|&ti| {
    ...
    let z = trapezoid(&unnorm, t);
    let z_safe = z + 1.0e-100;
    unnorm.into_iter().map(|p| p / z_safe).collect()
}
...
fn f_g_diff(ea: f64, t: &[f64], y: &[f64], beta: f64) -> f64 {
    let eps = 1e-10;
    let a: f64 = t.iter().zip(y.iter()).map(|(&ti, &yi)| yi / (KB * ti)).sum::<f64>() + eps;
    let b: f64 = y.iter().sum::<f64>() + eps;
    let g_sum: f64 = t.iter().zip(y.iter()).map(|(&ti, &yi)| {
...
pub fn mle_tsdc_lbfgsb(
    ...
    if let Ok(state) = lbfgsb::lbfgsb(init, &bounds, |p, g| {
        ...
        let d3: f64 = t_s.iter().zip(w_s.iter()).map(|(&ti, &yi)| yi * ti * expn(2, ea_c * INV_KB / ti)).sum();
        let d3_p: f64 = t_s.iter().zip(w_s.iter()).map(|(&ti, &yi)| -yi * INV_KB * expn(1, ea_c * INV_KB / ti)).sum();
